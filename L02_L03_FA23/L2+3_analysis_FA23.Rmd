...
```{r}
dat<-rbind(l2, l3 %>% unite(Flower, c("Flower", "Trial Number"), sep="."))
...
artificial<-ggplot(data=dat[(nrow(l2)+1):52,] %>% pivot_longer(colnames(dat[4:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Flower), aes(x=Flower, y=Observations, fill=Behavior))+
...
real_flowers_type<-ggplot(data=dat[1:nrow(l2),] %>% pivot_longer(colnames(dat[4:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Insect, Flower) %>% summarise(Total=sum(Observations)), aes(x=Flower, y=Total, fill=Insect))+
  geom_col(position="dodge")+
  ggtitle("Types of Insects Observed on Real Flowers")


real_flowers_type

artificial_type<-ggplot(data=dat[(nrow(l2)+1):52,] %>% pivot_longer(taxa, names_to="Insect", values_to="Observations") %>% group_by(Insect, Flower) %>%  summarise(Total=sum(Observations)), aes(x=Flower, y=Total))+
  geom_col(position="dodge", aes(fill=Insect)) + theme(axis.text.x=element_text(angle=45, hjust=1), axis.title.x = element_text(vjust=0.5)) + ggtitle("Types of Insects Observed on Artificial Flowers")

artificial_type

artificial_type<-ggplot(data=dat[(nrow(l2)+1):52,] %>% pivot_longer(taxa, names_to="Insect", values_to="Observations") %>% group_by(Insect, Flower) %>%  summarise(Total=sum(Observations)), aes(x=Flower, y=Total))+
  geom_col(position="dodge", aes(fill=Insect)) + theme(axis.text.x=element_text(angle=45, hjust=1), axis.title.x = element_text(vjust=0.5)) + ggtitle("Total Insects on Artificial Flowers by Type")
...
combined_type<-ggplot(data=dat %>% pivot_longer(colnames(dat[4:ncol(dat)]), names_to="Insect", values_to="Observations") %>% group_by(Type, Insect) %>% summarise(Average=mean(Observations), SE=se(Observations)), aes(x=Type, y=Average, fill=Insect))+
  ...
  labs(title="Average Number of Observed Insects on Real and Artificial Flowers", x="Flower Type", y="Average Number of Observations") + geom_errorbar(ymin=tax_lower, ymax=tax_upper, position = "dodge")
...
broad_v_target<-ggplot(data=SumStats_artificial, aes(x=Flower, y=Average, fill=Insect))+
  geom_col(position="dodge") + ylim(0,6) + geom_errorbar(ymin=art_lower, ymax=art_upper, position="dodge") + labs(title="Average Number of Interactions by Each Insect Type on Artificial Flowers", y="Average number of Interactions", x="Artificial Flower Design") + scale_fill_brewer(palette="Accent")
